...
- **Exit (c):** append a schema-conformant queued entry to `{{PROJECT_DIR}}/.codearbiter/open-tasks.md` (the format the SessionStart hook{{IF:claude}} and statusline parse{{ELSE}} parses{{END}} — see the `OPEN_TASKS` scaffold note): a top-level `- [ ] <symptom> (logged YYYY-MM-DD)` line, with the "no action" rationale as an indented `- Desc:` sub-bullet. A dotted ID is optional for a debug note. This keeps the note in the in-flight count as an open item to revisit, rather than a bare bullet that drifts from the schema. No further handoff.
