...
- `{{PLUGIN_ROOT}}/skills/commit-gate/SKILL.md` and `{{PLUGIN_ROOT}}/skills/tdd/SKILL.md` — the canonical v2 format the authored skill must mirror. Read them before Phase 3. The command-backed commit owner also demonstrates the
...
- **Format conformance** — frontmatter is `name`+`description`, plus `disable-model-invocation: true` on a chain-internal skill (ADR-0028);
  ...
  needs explicit generator support, never silent dropping; H1 matches `name`; phases are numbered with `· gate:`; paths use `{{PLUGIN_ROOT}}` / `{{PROJECT_DIR}}` correctly; no cut docs/skills, no legacy `${FRAMEWORK_ROOT}`/`${PROJECT_ROOT}`/`.agents/` paths.
...
- Add a row to `{{PLUGIN_ROOT}}/skills/INDEX.md`: skill name (linked), "Routed to by", and "Owns" (the one-sentence responsibility from Phase 2).
