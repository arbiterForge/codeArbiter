...
Natural-language requests and the retained `{{CMD:reconcile}}` entry use this
...
- Recording an already-made new ADR, or inspecting ADR health without arbitration,
  belongs to `{{PLUGIN_ROOT}}/skills/decision-lifecycle/SKILL.md`. Load that owner
...
- A rule conflict that prevents safe continuation uses `{{CMD:conflict}}`, not
...
`{{PLUGIN_ROOT}}/skills/decision-variance/references/analysis.md` once. It owns
...
`{{PLUGIN_ROOT}}/includes/smarts/decision-log-format.md`. Capture each choice
...
- **Supersede**: append the explicit replacement decision with the prior log ID in
  ...
  `{{PLUGIN_ROOT}}/skills/decision-lifecycle/SKILL.md`, with that user's decision,
...
- **Defer**: record the explicit deferral and re-evaluation trigger using the same
  ...
  question to `{{PROJECT_DIR}}/.codearbiter/open-questions.md` using the existing
...
`decision-challenger` (`{{PLUGIN_ROOT}}/agents/decision-challenger.md`) remains
