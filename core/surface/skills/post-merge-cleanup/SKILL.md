...
**It is ordinary lifecycle work and never needs `{{CMD:override}}`.** When it
...
`{{CMD:chore}}`, then for `{{CMD:override}}` — a bypass manufactured to cover a
coverage hole.

<!-- catalog-compatibility-notice:start -->
> Compatibility route. Prefer `{{CMD:pr}} --cleanup` for new explicit usage. The retained entry
...
Select this owner for an already-merged branch cleanup request, `{{CMD:pr}} --cleanup`,
or the explicit `{{CMD:cleanup}}` compatibility entry. Do not require repeated slash
...
MUST NOT require `{{CMD:override}}` for ordinary cleanup; name a real blocker instead.
