...
- `{{PROJECT_DIR}}/.codearbiter/CONTEXT.md` — confirm the repo is a git repo and the base/working branch.
- The discovered authoritative spec/plan pair for the caller's pipeline. Use its
  ...
  `{{PLUGIN_ROOT}}/includes/artifacts.md` and obtain
...
- Record each worktree path and branch in a dedicated manifest `{{PROJECT_DIR}}/.codearbiter/.worktrees.json` (gitignored scratch — NOT the `open-tasks.md` backlog) so a crash leaves no untracked isolation.
...
`{{PROJECT_DIR}}/.codearbiter/.worktrees.json`. A unit whose branch was integrated onto the
