...
- `{{CMD:adr-status}}` selects **status only**, with no arguments or `--adr N`.
...
- `{{CMD:adr}} "<decision title>"` selects authoring, including a title named
...
load `{{PLUGIN_ROOT}}/skills/decision-lifecycle/references/authoring.md` and follow
