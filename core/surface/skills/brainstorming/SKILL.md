...
`{{PLUGIN_ROOT}}/includes/artifacts.md` before artifact I/O. Before Phase 1, call
...
New full-lane work uses `{{PROJECT_DIR}}/.codearbiter/specs/<slug>.html` through the installed
...
3. **Approval:**
   ...
   - **Initial `/sprint` planning** — present the spec and adversarial findings for explicit user
     ...
     `{{PLUGIN_ROOT}}/includes/artifacts.md`: arm the exact current spec, require the exact returned reply,
   ...
   - **Within an already-approved sprint** — use its existing delegation only for an in-scope choice.
     ...
     follow `{{PLUGIN_ROOT}}/includes/artifacts.md`. A SMARTS score or log entry is not an approval receipt.
...
`writing-plans` (`{{PLUGIN_ROOT}}/skills/writing-plans/SKILL.md`) when that is the caller's declared
...
- MUST, at exit, run the follow-up harvest (`{{PLUGIN_ROOT}}/includes/harvest.md`) in the active caller's mode:
