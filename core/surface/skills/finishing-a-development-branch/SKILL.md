...
`{{PLUGIN_ROOT}}/commands/watch.md` with the remaining arguments, then returns.
...
`--cleanup` loads and follows `{{PLUGIN_ROOT}}/skills/post-merge-cleanup/SKILL.md`
...
A direct request to open a PR, including `{{CMD:pr}} ["title"]`, selects **Open a PR**.
...
- The discovered authoritative spec/plan pair for the pipeline, when `/feature`
  ...
  than reconstructing either path. Load `{{PLUGIN_ROOT}}/includes/artifacts.md`
...
- **ADR source ancestry** — when `.codearbiter/decisions/adr-lifecycle.jsonl` exists, select
  ...
  `python "{{PLUGIN_ROOT}}/hooks/adr-merge-method.py" --root "{{PROJECT_DIR}}" --base-ref <base-sha> --current-ref <head-sha> --merge-method`.
...
1. **Confirm the commit gate cleared** this session (`commit-gate` green, or `{{CMD:commit}}` completed).
...
5. **Stage the PR** once all BLOCK findings clear: concise title; summary of what changed and why; a
   ...
   composing it, load `{{PLUGIN_ROOT}}/includes/anti-slop-design/core.md` and the
...
6. **Auto-attach the babysitter** — resolve the flag with the canonical resolver, never by eyeballing
   ...
   `{{CMD:watch}} <new-PR>`. When `enabled` is false, do nothing here — the user can still run `{{CMD:watch}}`
