...
loaded on demand when {{CMD:sprint}} (commands/sprint.md) is invoked; the command
...
> default. The fresh-evidence promotion bar lives in `{{PLUGIN_ROOT}}/includes/farm.md`.
...
citing `{{PLUGIN_ROOT}}/includes/farm.md`, before brainstorming begins. Everything else about
...
`{{PROJECT_DIR}}/.codearbiter/specs/<sprint-slug>.html` through the installed engine, then to `writing-plans` (`{{PLUGIN_ROOT}}/skills/writing-plans/SKILL.md`) for the task
breakdown at `{{PROJECT_DIR}}/.codearbiter/plans/<sprint-slug>.html`.
...
Hand the approved plan to `subagent-driven-development` (`{{PLUGIN_ROOT}}/skills/subagent-driven-development/SKILL.md`) and run it to completion WITHOUT per-batch
...
- Run the SMARTS 6-lens evaluation (the scoring in
  `{{PLUGIN_ROOT}}/includes/smarts/core.md`), beginning with its Step 0 recorded-intent check
...
- LOG every auto-decision to `{{PROJECT_DIR}}/.codearbiter/sprint-log.md` (append-only): the
