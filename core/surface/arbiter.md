...
- The user states a goal or **invokes** {{IF:claude}}`/command`{{ELSE}}`$ca-command`{{END}}; the orchestrator **routes** to its owner; a skill **dispatches** agents.
...
and `{{PLUGIN_ROOT}}/includes/routing-table.md` — load them when a scope-touch or route needs them,
...
3. **Genuinely unclear** — use `{{PLUGIN_ROOT}}/includes/redirect.md` to ask about the intended
...
**`{{CMD:btw}} "question"`** remains an explicit Q&A convenience, not a prerequisite for asking.
