...
The `decision-variance` skill (`{{PLUGIN_ROOT}}/skills/decision-variance/SKILL.md`). It locates
the three architectural artifacts by exact filename — `01-architecture-breakdown.md`,
`02-phased-build-plan.md`, `03-task-backlog.md` (under `{{PROJECT_DIR}}/.codearbiter/plans/`) —
and indexes the append-only decision log at
`{{PROJECT_DIR}}/.codearbiter/decisions/decision-log.md`. It MAY dispatch the
`decision-challenger` agent (`{{PLUGIN_ROOT}}/agents/decision-challenger.md`) to stress-test an
...
2. **Supersede** — a new decision is recorded by appending a log entry whose `Supersedes:` references
   the prior one; a replacement ADR is authored via `{{CMD:adr}}`.
3. **Surface as `[CONFIRM-NN]`** — unresolvable now; a numbered placeholder is added to
   `{{PROJECT_DIR}}/.codearbiter/open-questions.md`.

## When NOT to use

- Author a brand-new ADR with no prior conflict → `{{CMD:adr}}`.
- Check ADR health (aged, unchallenged, unresolved CONFIRM-NN) → `{{CMD:adr-status}}`.
- A general architectural discussion without recording a decision → `{{CMD:btw}}`.
- A rule conflict between project docs and code → `{{CMD:conflict}}`.
...
ADR as the disposition of a routine finding — ADRs are authored only via `{{CMD:adr}}` with user
attribution.
