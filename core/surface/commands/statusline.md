...
# {{CMD:statusline}} — wire-up

A plugin cannot own a `statusLine`, and `{{PLUGIN_ROOT}}` is not expanded inside
`settings.json`. So the renderer at `{{PLUGIN_ROOT}}/hooks/statusline.py` is wired in by
resolving its absolute path at install time and writing it to the user's `statusLine.command`. The
backing script `{{PLUGIN_ROOT}}/hooks/wire-statusline.py` performs the surgery deterministically
...
1. Run the backing script with the requested action, passing the resolved plugin root so the absolute
   ...
   ```
   python "{{PLUGIN_ROOT}}/hooks/wire-statusline.py" <action> --plugin-root "{{PLUGIN_ROOT}}"
   ```
