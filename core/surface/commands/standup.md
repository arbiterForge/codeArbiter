...
- A read-only state snapshot without acting → `{{CMD:status}}`.
- Install health (interpreter, payload, hooks) → `{{CMD:doctor}}`.
{{IF:claude}}
- Transcript hygiene to extend the session → `{{CMD:prune}}`.
{{END}}
- Committing staged work → `{{CMD:commit}}`.
...
- MUST NOT auto-flip any board entry during the drift sweep — the sweep is advisory
  ...
  exclusively through `{{CMD:task}} done <id>`.
