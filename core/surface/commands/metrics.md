...
# {{CMD:metrics}} — governance trend glance
...
This is NOT a second `{{CMD:audit}}` packet. It prints numbers and arrows only — no
verbatim override lines, no commit list, no file write. Use it to spot a trend at a
glance; reach for `{{CMD:audit}}` when you need the full evidentiary packet.
...
- MUST NOT require `{{CMD:init}}` to have been run. The helper degrades gracefully on
...
- Full governance packet with verbatim overrides and audit trail → `{{CMD:audit}}`.
- Live project state (active sprint, open confirms, hook health) → `{{CMD:status}}`.
