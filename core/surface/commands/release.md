...
3. **Surfaces** — roll the section into the declared changelog, update every declared manifest, and run the row's declared `pre-tag` checks. After those edits are committed, declared assets are qualified in the protected publisher before any tag push: either `release-build` produces an exact inventory in an empty hosted directory, or a reviewed retained exact-head CI cohort supplies provenance-bound packages with the same declared filenames. Tracked-tree mutation, arbitrary build failure, provenance mismatch, or inventory drift BLOCKs.
...
`{{CMD:release}} --dry-run [target]` runs the read-only, locally evaluable portion of Pre-flight, then Version's
...
`{{PROJECT_DIR}}/.codearbiter/release-targets.md` without tagging anything. Declared `pre-tag` checks
...
- Tagging an in-progress branch → land work first via `{{CMD:feature}}` / `{{CMD:fix}}`.
