...
Routes to the `release` skill. `{{CMD:release}}` targets the **`ca`** plugin only (ADR-0007): `LAST_TAG`
...
1. **Pre-flight** — working tree clean, on the configured release branch, suite green, no blocking
   `[CONFIRM-NN]` open, HEAD not produced via `{{CMD:override}}`. Identify `LAST_TAG` and the release window.
...
`release` (`{{PLUGIN_ROOT}}/skills/release/SKILL.md`).
...
explicit user authorization. Any BLOCK may be bypassed only via `{{CMD:override}}`.
