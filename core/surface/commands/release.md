...
# {{CMD:release}} — target-aware tagged release

The only permitted path to a version tag. A release is a deployment-readiness assertion for one
specific plugin payload at one commit. `{{CMD:release}}` routes to the `release` skill; it does not
...
{{CMD:release}}
{{CMD:release}} ca
{{CMD:release}} ca-codex
{{CMD:release}} ca-sandbox
{{CMD:release}} ca-pi
```
