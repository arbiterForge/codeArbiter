...
3. Write the packet to `{{PROJECT_DIR}}/.codearbiter/audits/<YYYY-MM-DD>.md` (second run the
...
- Live project state right now → `{{CMD:status}}`.
- Triggering reviews → `{{CMD:checkpoint}}` (this command only reports what reviews already found).
