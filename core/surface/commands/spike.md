---
description: Exploratory spike on a throwaway branch — answer a named question with disposable code. Never merges; exits to a findings note or {{CMD:feature}}.
argument-hint: "<question to answer> [timebox]"
---

# {{CMD:spike}} — exploratory spike
...
- You already know what to build → `{{CMD:feature}}`.
- Diagnosing a defect → `{{CMD:debug}}` (investigation with a structured exit).
- A question answerable by reading code or docs → `{{CMD:btw}}`.
