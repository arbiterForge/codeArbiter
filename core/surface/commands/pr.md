...
4. **BLOCK check** — any CRITICAL or HIGH finding STOPs the flow; present it and do not draft the PR.
   The user resolves, re-runs `{{CMD:commit}}`, then `{{CMD:pr}}`.
...
`finishing-a-development-branch` (`{{PLUGIN_ROOT}}/skills/finishing-a-development-branch/SKILL.md`),
open-PR path.

## When NOT to use

- Staged changes not yet committed → `{{CMD:commit}}`.
- Review a diff without opening a PR → `{{CMD:review}}`.
- A pre-implementation security pass → `{{CMD:threat-model}}`.
