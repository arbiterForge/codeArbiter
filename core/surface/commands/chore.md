...
# {{CMD:chore}} — non-behavioral change lane
...
- **Anti-slop copy pass** — for any user-facing doc the change touches (repo-root community docs
  ...
  `{{PLUGIN_ROOT}}/includes/anti-slop-design/INDEX.md`, then `core.md` and the
...
the other).
- Gates: dispatch `dependency-reviewer` (same vetting as `{{CMD:add-dep}}` — license, provenance,
  supply chain, changelog of the bump); affected local tests green under
  `{{PLUGIN_ROOT}}/includes/verification-boundary.md`; exhaustive exact-head hosted CI before merge;
  `commit-gate`.
- A bump that requires code changes to adopt is not a chore — route the code change through
  `{{CMD:feature}}` or `{{CMD:fix}}`.
...
- No new regression test demanded — the revert restores already-tested behavior. If the revert is
  fixing a defect the suite missed, open `{{CMD:fix}}` afterward to pin it.

## Flow

1. Classify `$ARGUMENTS` into one of the three types. Anything with a behavioral code change beyond
   a mechanical revert is NOT a chore — redirect to `{{CMD:feature}}` or `{{CMD:fix}}` and stop.
...
MUST reject a change containing behavioral code (beyond the revert itself) — that is `{{CMD:feature}}`
or `{{CMD:fix}}` territory. MUST keep manifest and lockfile changes in the same commit for `deps`.
...
- Any new or changed behavior → `{{CMD:feature}}` / `{{CMD:fix}}`.
- Adding a NEW dependency → `{{CMD:add-dep}}`.
- Restructuring code → `{{CMD:refactor}}`.
