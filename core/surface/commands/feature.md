...
# {{CMD:feature}} — spec-driven feature
...
`{{PLUGIN_ROOT}}/includes/artifacts.md` before resume classification or artifact
...
Before triage, scan `{{PROJECT_DIR}}/.codearbiter/specs/` and `plans/` for artifacts
...
3. **Spec approved, no plan** → `writing-plans` (`{{PLUGIN_ROOT}}/skills/writing-plans/SKILL.md`).
4. **Spec exists but never approved** → `brainstorming` (`{{PLUGIN_ROOT}}/skills/brainstorming/SKILL.md`), at its approval gate — not from scratch.
...
confirmation. On confirmation, append one line to `{{PROJECT_DIR}}/.codearbiter/triage.log`
(append-only, `>>`):
...
Then route directly to `tdd` (`{{PLUGIN_ROOT}}/skills/tdd/SKILL.md`) — the confirmed criteria are its Phase 1 obligations; Phases 2–6 run
...
1. **`brainstorming`** (`{{PLUGIN_ROOT}}/skills/brainstorming/SKILL.md`) — refine `$ARGUMENTS` into a concrete spec by Socratic questioning: challenge
   vague language, surface hidden complexity, force trade-offs. Writes the spec to
   `{{PROJECT_DIR}}/.codearbiter/specs/<slug>.html` through the installed engine. **Hard gate: no plan and no code until the
   user approves the spec.** Genuinely-unresolved unknowns become `[CONFIRM-NN]` in
   `open-questions.md`, never guesses.
2. **`writing-plans`** (`{{PLUGIN_ROOT}}/skills/writing-plans/SKILL.md`) — decompose the approved spec into small tasks, each with an exact path and a
   verification that maps to a `tdd` obligation (it does not replace one). Writes
   `{{PROJECT_DIR}}/.codearbiter/plans/<slug>.html` with bijective criterion↔task coverage.
3. **`executing-plans`** (`{{PLUGIN_ROOT}}/skills/executing-plans/SKILL.md`) — coordinates the plan in small batches with human checkpoints. Each batch is
   delegated to `subagent-driven-development` (`{{PLUGIN_ROOT}}/skills/subagent-driven-development/SKILL.md` — fresh author agent per task, spec-compliance review,
...
The autonomous counterpart (`{{CMD:sprint}}`) runs the same spec→plan but passes the full plan to
...
tests from `{{PLUGIN_ROOT}}/includes/verification-boundary.md` to be green; exhaustive cross-area and
