...
The `decision-lifecycle` skill (`{{PLUGIN_ROOT}}/skills/decision-lifecycle/SKILL.md`). The skill
owns numbering, the file template, the `proposed → accepted → superseded | rejected` status lifecycle,
and writes the file to `{{PROJECT_DIR}}/.codearbiter/decisions/`. Status transitions require
explicit user instruction; the orchestrator never advances an ADR on its own.

## When NOT to use

- Check the health of existing ADRs → `{{CMD:adr-status}}`.
- Reconcile or challenge a suspect ADR → `{{CMD:reconcile}}`.
- Ask about a decision without recording it → `{{CMD:btw}}`.

## Hard gate

An ADR is authored ONLY via `{{CMD:adr}}` with explicit user attribution. MUST NOT author an ADR as the
