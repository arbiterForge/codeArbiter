...
`{{PLUGIN_ROOT}}/commands/context-check.md`. The audit begins read-only; re-scout or re-baseline may
...
2. **Pipelines** — every slug in `{{PROJECT_DIR}}/.codearbiter/specs/` and `plans/`. Select the
   ...
   An interrupted pipeline is resumable via `{{CMD:feature}}` — say so on its line.
