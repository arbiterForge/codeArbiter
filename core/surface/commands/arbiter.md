...
# {{CMD:arbiter}} — restore orchestration

The exit door for `{{CMD:dev}}`. No-op if dev mode is not active.

## Flow

1. **Log exit** — append to `{{PROJECT_DIR}}/.codearbiter/overrides.log` (append-only, `>>`):

   ```
   [ISO-8601 timestamp] | BY: <email> | DEV: exit
   ```

2. **Marker** — remove `{{PROJECT_DIR}}/.codearbiter/.markers/dev-active`{{IF:claude}}; the statusline
   returns to normal{{END}}.
