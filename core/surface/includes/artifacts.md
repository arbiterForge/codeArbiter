...
method under its existing grant, load `{{PLUGIN_ROOT}}/includes/sprint-authority.md`. That private
...
```sh
python "{{PLUGIN_ROOT}}/hooks/artifact-authority.py" arm --root "{{PROJECT_DIR}}" --artifact-id <plan-id> --record-id <task-id> --activity verification --workspace "candidate worktree=<absolute-worktree>" --workspace "baseline-evidence worktree=<absolute-worktree>"
```
...
python "{{PLUGIN_ROOT}}/hooks/artifact-authority.py" publish --root "{{PROJECT_DIR}}" --request-id <request-id>
```
...
```sh
python "{{PLUGIN_ROOT}}/hooks/artifact-reconcile.py" --root "{{PROJECT_DIR}}" --artifact-id <plan-id> --target-id <task-id> --operation task-reconcile --target-state PENDING --reason "<reviewed reason>" --assessment "<reviewed assessment>"
python "{{PLUGIN_ROOT}}/hooks/artifact-reconcile.py" --root "{{PROJECT_DIR}}" --artifact-id <plan-id> --target-id <checkpoint-id> --operation scope-reconcile --assessment "<reviewed assessment>"
```
...
```sh
python "{{PLUGIN_ROOT}}/hooks/artifact-reconcile.py" --root "{{PROJECT_DIR}}" --artifact-id <plan-id> --cancel --prompt "<exact returned reply>"
```
...
```sh
python "{{PLUGIN_ROOT}}/hooks/artifact-reconcile.py" --root "{{PROJECT_DIR}}" --artifact-id <plan-id> --cancel-orphan
```
...
```sh
python "{{PLUGIN_ROOT}}/hooks/artifact-reconcile.py" --root "{{PROJECT_DIR}}" --artifact-id <plan-id> --recover
```
