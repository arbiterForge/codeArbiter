...
when the user invokes {{CMD:dev}} or {{CMD:arbiter}}. The arbiter mode body (arbiter.md)
...
with no orchestration mediating — skill, agent, command, and hook bodies, `arbiter.md`, settings.
...
- **Log:** on entry, append `[ISO-8601] | BY: <git user.email> | DEV: enter | NOTE: <note or —>` to
  `.codearbiter/overrides.log` (append with `>>`, per arbiter.md §7's append-only rule). On exit,
