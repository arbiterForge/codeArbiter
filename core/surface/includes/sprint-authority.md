...
```sh
python "{{PLUGIN_ROOT}}/hooks/_approvallib.py" arm-sprint --root "{{PROJECT_DIR}}" --spec-id <spec-id> --plan-id <plan-id> --delegate-methods
```
...
```sh
python "{{PLUGIN_ROOT}}/hooks/_approvallib.py" resume-sprint --root "{{PROJECT_DIR}}" --plan-id <plan-id>
```
...
```sh
python "{{PLUGIN_ROOT}}/hooks/_approvallib.py" smarts --root "{{PROJECT_DIR}}" --request-file <decision-request.json>
```
