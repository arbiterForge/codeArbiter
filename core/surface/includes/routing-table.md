...
| Add / start / done a backlog task | `{{CMD:task}}` → `hooks/taskwrite.py` (the pure `_taskboardlib` transforms) | — | Board written only through the writer; `start` always stamps a date; never delete to complete |
