...
is clear: `/ca:override`, merge to the default branch, branch or worktree deletion, and release
...
capability; never steer the user toward `/ca:override` to get past a coverage hole. Do not invent
an ungoverned procedure, resolve a `[CONFIRM-NN]`, or broaden scope merely to avoid asking.

## Exception — local runtime work

The existing `mode --ops` contract in `${CLAUDE_PLUGIN_ROOT}/includes/ops-mode.md` owns local runtime
