...
```sh
python "${CLAUDE_PLUGIN_ROOT}/hooks/_approvallib.py" arm-sprint --root "${CLAUDE_PROJECT_DIR}" --spec-id <spec-id> --plan-id <plan-id> --delegate-methods
```
...
```sh
python "${CLAUDE_PLUGIN_ROOT}/hooks/_approvallib.py" resume-sprint --root "${CLAUDE_PROJECT_DIR}" --plan-id <plan-id>
```
...
```sh
python "${CLAUDE_PLUGIN_ROOT}/hooks/_approvallib.py" smarts --root "${CLAUDE_PROJECT_DIR}" --request-file <decision-request.json>
```
