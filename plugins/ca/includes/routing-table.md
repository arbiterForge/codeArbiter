...
Routing to a skill means loading its body from `${CLAUDE_PLUGIN_ROOT}/skills/<name>/SKILL.md` — a route
...
| Behavior-preserving restructure | `refactor` skill at `${CLAUDE_PLUGIN_ROOT}/skills/refactor/SKILL.md` (explicit `/refactor` uses the same owner) | `tdd` Phase 1 (coverage backfill or new seams) | No refactor without parity-coverage proof |
| Unknown defect / investigation | `debug` skill at `${CLAUDE_PLUGIN_ROOT}/skills/debug/SKILL.md` (explicit `/debug` uses the same owner) | — | No code change in the skill; one named exit |
| Commit | `commit-gate` skill at `${CLAUDE_PLUGIN_ROOT}/skills/commit-gate/SKILL.md` (explicit `/commit` uses the same owner) | — | No commit without all nine gates green |
| Open a PR / finish a branch | `finishing-a-development-branch` at `${CLAUDE_PLUGIN_ROOT}/skills/finishing-a-development-branch/SKILL.md`; select direct-open or caller mode first | reviewer fleet per path; PR-body prose applies `anti-slop-design` (`core` + `medium-documents` §7.A.1) | PR only; no direct-to-default, no force-push |
...
| Requested threat model of a sensitive design | `security-architecture` at `${CLAUDE_PLUGIN_ROOT}/skills/security-architecture/SKILL.md` (opt-in; explicit `/threat-model` retained) | `security-reviewer`, `auth-crypto-reviewer` | STOP only on a critical unmitigated threat |
| Arbitration / variance / ADR reconciliation | `decision-variance` at `${CLAUDE_PLUGIN_ROOT}/skills/decision-variance/SKILL.md` (explicit `/ca:reconcile` retained); select report-only or reconciliation first | `scout`, `grader`, `decision-challenger` (optional, read-only) | No decision recorded without explicit user choice and attribution; report-only writes nothing |
| New / aged ADR, unresolved `[CONFIRM-NN]` | `decision-lifecycle` at `${CLAUDE_PLUGIN_ROOT}/skills/decision-lifecycle/SKILL.md`; select author or read-only status before prerequisites | `decision-challenger` (optional) | No `[CONFIRM-NN]` resolved by guessing |
...
| Finish an already-merged branch (post-merge cleanup) | `post-merge-cleanup` at `${CLAUDE_PLUGIN_ROOT}/skills/post-merge-cleanup/SKILL.md` (explicit `/ca:cleanup` or `/ca:pr --cleanup`) | — | Merge proven against the FETCHED default by ancestry, PR-record squash proof, or a byte-identical fallback; unique/unclassifiable artifacts never discarded without a per-item confirmation; `--ff-only`; `branch -d`, or `-D` only under the restated squash-proof path; remote branch untouched; never routes to `/ca:override` |
...
| Manual drift audit of provenance-tracked docs (bypass / merge / external edit case) | `context-check` at `${CLAUDE_PLUGIN_ROOT}/skills/context-check/SKILL.md` (explicit `/ca:status drift` or `/ca:context-check`) | — | Read-only unless user selects re-scout or re-baseline; MUST NOT commit on its own; not the daily loop — commit-gate auto-heal is the routine path |
| Deep on-demand codebase audit | `tribunal` at `${CLAUDE_PLUGIN_ROOT}/skills/tribunal/SKILL.md` (explicit `/ca:tribunal` retained) | `tribunal-lens-reviewer` per active lens (+ optional `map-structure`/`map-deps` on large repos) | Read-only until the filing gate; runs only after the user acknowledges the token estimate; never blocks a merge/commit; nothing filed or sent without explicit authorization |
