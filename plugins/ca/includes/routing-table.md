...
| Deep periodic codebase audit | `/ca:tribunal` → `tribunal` skill | ten `tribunal-*` reviewers (+ optional `map-structure`/`map-deps` on large repos) | Read-only until the filing gate; runs only after the user acknowledges the token estimate; never blocks a merge/commit; nothing filed or sent without explicit authorization |
