...
description: Dispatched by the tribunal deep-audit lane for the coverage lens. Read-only review of risk-path coverage gaps, edge/property gaps, and implementation-coupled tests. Appends findings as jsonl.
...
## Findings
Append one finding/v1 line to `findings/coverage.jsonl` the moment it is found — never batch. Provisional scores only.
