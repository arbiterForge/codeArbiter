...
5. `${CLAUDE_PLUGIN_ROOT}/includes/author-tdd-workflow.md` — the six-step TDD execution order for every task. Read it; do not carry a remembered copy.

## TDD Workflow (Non-Negotiable)

Follow the six-step fixed order in `${CLAUDE_PLUGIN_ROOT}/includes/author-tdd-workflow.md` for every task — failing tests first, minimum implementation, full suite, lint/type-check, only then stage.
