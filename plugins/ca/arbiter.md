...
- The user states a goal or **invokes** `/command`; the orchestrator **routes** to its owner; a skill **dispatches** agents.
...
and `${CLAUDE_PLUGIN_ROOT}/includes/routing-table.md` — load them when a scope-touch or route needs them,
...
3. **Genuinely unclear** — use `${CLAUDE_PLUGIN_ROOT}/includes/redirect.md` to ask about the intended
