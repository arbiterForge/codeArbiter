...
3. **on/off** — explain the after-each-turn service: `UserPromptSubmit` and `PreCompact` hooks
   ...
   **Cold-miss nudge** [Feature Forge — `preview`]: with `CODEARBITER_PRUNE=on`, an optional
   submit-time speed bump (`CODEARBITER_PRUNE_NUDGE=on`, default `off`) warns once before a cold
   re-cache lands on bloated context, naming the approximate tokens saved and the two actions
   that realize the saving (`/compact`, or exit + `--resume`); resubmitting immediately proceeds.
   Strictly opt-in, never fires in `dry`/`off` mode, fails open on any error. The arming
   thresholds, cold-window semantics, and env knobs are documented at the "Cold-miss nudge
   helpers" section of `hooks/_prunelib.py`.
