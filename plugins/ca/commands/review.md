...
Read-only review of a change. Routes to `dispatching-parallel-agents` (`${CLAUDE_PLUGIN_ROOT}/skills/dispatching-parallel-agents/SKILL.md`): dispatches the reviewer fleet by path matrix, dedupes, then funnels through `finding-triage` → `verdict-aggregator` to a single in-memory verdict. No code is modified.
