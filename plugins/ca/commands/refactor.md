...
3. **Red parity tests (conditional)** — if the refactor exposes a new test seam, route to `tdd` (`${CLAUDE_PLUGIN_ROOT}/skills/tdd/SKILL.md`)
...
5. **Parity verification** — every impact-bounded local parity test identified for the surface passes
   ...
   `${CLAUDE_PLUGIN_ROOT}/includes/verification-boundary.md` reserves the exhaustive suite for exact-head
