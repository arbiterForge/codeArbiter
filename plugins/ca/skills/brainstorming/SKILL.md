...
`${CLAUDE_PROJECT_DIR}/.codearbiter/specs/<slug>.html`; do not write the HTML or its
...
On approval, hand off to the `tdd` skill (`${CLAUDE_PLUGIN_ROOT}/skills/tdd/SKILL.md`), which enters Phase 1 against the approved spec — one obligation per acceptance criterion.
