...
`${CLAUDE_PLUGIN_ROOT}/includes/artifacts.md` before artifact I/O. Before Phase 1, call
...
Record every genuinely-unresolved user-owned requirement or decision as `[CONFIRM-NN]` in `${CLAUDE_PROJECT_DIR}/.codearbiter/open-questions.md`, numbered sequentially. A finding that belongs to a different feature or a future scope gets an inline `[NEEDS-TRIAGE]` marker in the notes — never route it to a ticket.
...
New full-lane work uses `${CLAUDE_PROJECT_DIR}/.codearbiter/specs/<slug>.html` through the installed
...
3. **Approval:**
   ...
   - **Initial `/sprint` planning** — present the spec and adversarial findings for explicit user
     ...
     `${CLAUDE_PLUGIN_ROOT}/includes/artifacts.md`: arm the exact current spec, require the exact returned reply,
   ...
   - **Within an already-approved sprint** — use its existing delegation only for an in-scope choice.
     ...
     follow `${CLAUDE_PLUGIN_ROOT}/includes/artifacts.md`. A SMARTS score or log entry is not an approval receipt.
...
`writing-plans` (`${CLAUDE_PLUGIN_ROOT}/skills/writing-plans/SKILL.md`) when that is the caller's declared
...
- MUST, at exit, run the follow-up harvest (`${CLAUDE_PLUGIN_ROOT}/includes/harvest.md`) in the active caller's mode:
