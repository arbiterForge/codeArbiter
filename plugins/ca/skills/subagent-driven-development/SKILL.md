...
- The discovered authoritative spec/plan pair. Existing Markdown pairs retain
  their current ledger. For HTML, load `${CLAUDE_PLUGIN_ROOT}/includes/artifacts.md`,
...
The subagent works test-first by routing through the `tdd` skill (`${CLAUDE_PLUGIN_ROOT}/skills/tdd/SKILL.md`) — no implementation code before
...
Under an approved `/sprint`, an ordinary implementation/test failure routes to **Recovery within the approved sprint** in `${CLAUDE_PLUGIN_ROOT}/SPRINT.md`; rerun the original gate after an evidence-led correction.
...
- MUST halt and surface a real authority or security block, including security CRITICAL findings and
  ...
  under an approved sprint take Recovery within the approved sprint in `${CLAUDE_PLUGIN_ROOT}/SPRINT.md`.
