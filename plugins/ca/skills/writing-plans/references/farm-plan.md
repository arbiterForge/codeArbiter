...
Apply the shared `${CLAUDE_PLUGIN_ROOT}/includes/verification-boundary.md`: the slice's narrow RED test and
...
`${CLAUDE_PROJECT_DIR}/.codearbiter/plans/<slug>.plan.json` and an immutable content-addressed base binding
