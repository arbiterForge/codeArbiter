...
For a new full-lane HTML spec, load `${CLAUDE_PLUGIN_ROOT}/includes/artifacts.md` and
...
- The authoritative approved spec selected by `/feature`: for HTML, resolve it
  ...
  for legacy Markdown, read `${CLAUDE_PROJECT_DIR}/.codearbiter/specs/<slug>.md`.
...
`${CLAUDE_PROJECT_DIR}/.codearbiter/plans/<slug>.md` — `<slug>` matching the spec —
...
`${CLAUDE_PLUGIN_ROOT}/skills/writing-plans/references/farm-plan.md`. Load that leaf and follow it;
