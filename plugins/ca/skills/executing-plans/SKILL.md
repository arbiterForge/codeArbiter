...
- The discovered authoritative spec/plan pair. Existing Markdown pairs retain
  ...
  `${CLAUDE_PLUGIN_ROOT}/includes/artifacts.md`, retain the exact artifact identities
...
completion. Use the installed reconciliation adapter in `${CLAUDE_PLUGIN_ROOT}/includes/artifacts.md` for the exact task,
...
Invoke `subagent-driven-development` (`${CLAUDE_PLUGIN_ROOT}/skills/subagent-driven-development/SKILL.md`) with `scope = [current batch task IDs]`. Pass the plan slug and
