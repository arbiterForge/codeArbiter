...
Read `${CLAUDE_PLUGIN_ROOT}/includes/smarts/decision-log-format.md` before any log append.
...
- `${CLAUDE_PROJECT_DIR}/.codearbiter/decisions/` — if present, read the directory and
...
```bash
ADR_MARKER_ROOT="$("$PY" -c 'import sys; from pathlib import Path; sys.path.insert(0, sys.argv[1]); from _hooklib import project_root; print(Path(project_root()).as_posix())' "${CLAUDE_PLUGIN_ROOT}/hooks")" || exit 1
...
3. From that exact commit, hash the committed Git blob bytes and the separately canonicalized
   ...
   `acceptance` event to
   `${CLAUDE_PROJECT_DIR}/.codearbiter/decisions/adr-lifecycle.jsonl`, then persist that acceptance binding
