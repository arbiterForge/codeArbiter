...
Read `${CLAUDE_PLUGIN_ROOT}/includes/smarts/core.md` before Phase 3.

## Pre-flight - scope before prerequisites

Read `${CLAUDE_PROJECT_DIR}/.codearbiter/CONTEXT.md` and its `stage:` value. Unreadable or
missing required context is reported as a gap, never initialized by this pass.
Read `${CLAUDE_PROJECT_DIR}/.codearbiter/security-controls.md` only when a variance touches
...
Search `${CLAUDE_PROJECT_DIR}/.codearbiter/plans/` first, then the project root, then
...
`${CLAUDE_PROJECT_DIR}/.codearbiter/decisions/decision-log.md`, and relevant scaffold
...
SHA-256 using `${CLAUDE_PLUGIN_ROOT}/includes/smarts/decision-log-format.md`: heading
