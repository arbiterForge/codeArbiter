...
`<plugin-root>/skills/ca-watch/SKILL.md` with the remaining arguments, then returns.
...
`--cleanup` loads and follows `<plugin-root>/routines/post-merge-cleanup/SKILL.md`
...
- The discovered authoritative spec/plan pair for the pipeline, when `/feature`
  ...
  than reconstructing either path. Load `<plugin-root>/includes/artifacts.md`
