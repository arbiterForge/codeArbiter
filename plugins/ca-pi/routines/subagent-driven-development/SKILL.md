...
- The discovered authoritative spec/plan pair. Existing Markdown pairs retain
  their current ledger. For HTML, load `<plugin-root>/includes/artifacts.md`,
...
Under an approved `/sprint`, an ordinary implementation/test failure routes to **Recovery within the approved sprint** in `<plugin-root>/SPRINT.md`; rerun the original gate after an evidence-led correction.
...
- MUST halt and surface a real authority or security block, including security CRITICAL findings and
  ...
  under an approved sprint take Recovery within the approved sprint in `<plugin-root>/SPRINT.md`.
