...
For an HTML spec/plan pair, load `<plugin-root>/includes/artifacts.md` and use
...
- The authoritative approved spec selected by `/feature`. For HTML, read its
  ...
  `<project-root>/.codearbiter/specs/<slug>.md`. It is the primary obligation
  source.
...
`<plugin-root>/includes/maturity-coverage.md`, then run its coverage command.
