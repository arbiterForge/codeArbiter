...
`<project-root>/.codearbiter/specs/<sprint-slug>.html` through the installed engine, then to `writing-plans` (`<plugin-root>/routines/writing-plans/SKILL.md`) for the task
breakdown at `<project-root>/.codearbiter/plans/<sprint-slug>.html`.
