...
> default. The fresh-evidence promotion bar lives in `<plugin-root>/includes/farm.md`.
...
`<plugin-root>/includes/artifacts.md` owns the combined one-reply transaction:
...
Use the private request shape and adapter in `<plugin-root>/includes/artifacts.md`; a pasted matrix
...
For HTML, load `<plugin-root>/includes/artifacts.md` and use the exact current identity,
