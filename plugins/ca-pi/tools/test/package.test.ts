...
async function createPinnedGitFixture(root: string, environment: NodeJS.ProcessEnv) {
  ...
  await writeFile(
    ...
    '{"name":"@earendil-works/pi-coding-agent","version":"0.84.1","type":"module","bin":{"pi":"dist/cli.js"},"exports":{".":{"import":"./dist/index.js"}}}\n',
    "utf8",
  );
  await writeFile(resolve(poisonRoot, "dist", "cli.js"), "// poisoned fake CLI anchor\n", "utf8");
  await writeFile(
    resolve(poisonRoot, "dist", "index.js"),
    'globalThis.__CA_PI_POISON_HOST_EVALUATED__ = true; console.error("COUNTERFEIT_HOST_RUNTIME_EVALUATED"); export class ModelRegistry {} export const VERSION = "0.84.1";\n',
    "utf8",
  );
  await writeFile(
  ...
  await writeFile(
    ...
    '{"name":"not-pi","version":"0.84.1","type":"module","bin":{"pi":"dist/cli.js"},"exports":{".":{"import":"./dist/index.js"}}}\n',
    "utf8",
  );
  await writeFile(resolve(wrongRoot, "dist", "cli.js"), "// wrong package CLI\n", "utf8");
  await writeFile(resolve(wrongRoot, "dist", "index.js"), "export class ModelRegistry {} export const VERSION = '0.84.1';\n", "utf8");
  await writeFile(
    resolve(escapeRoot, "package.json"),
    '{"name":"@earendil-works/pi-coding-agent","version":"0.84.1","type":"module","bin":{"pi":"dist/cli.js"},"exports":{".":{"import":"../outside-runtime.js"}}}\n',
    "utf8",
  );
  await writeFile(resolve(escapeRoot, "dist", "cli.js"), "// escaping export CLI\n", "utf8");
  await writeFile(resolve(extensionRoot, "outside-runtime.js"), "export class ModelRegistry {} export const VERSION = '0.84.1';\n", "utf8");
  if (process.platform !== "win32") {
...
describe("ca-pi package", () => {
  ...
  test("real Pi loader rejects ordinary duplicate-host resolution and stays offline/auth-isolated", async () => {
    const execution = await withFixtureCleanupBoundary(async (resources) => {
      ...
      for (const input of [
        ...
        { piVersion: "0.84.1", nodeVersion: "24.16.0", pythonMajor: 3 },
        ...
        { piVersion: "0.84.1-rc.1", nodeVersion: "24.16.0", pythonMajor: 3 },
        { piVersion: "0.84.1+build.1", nodeVersion: "24.16.0", pythonMajor: 3 },
        { piVersion: "v0.84.1", nodeVersion: "24.16.0", pythonMajor: 3 },
        { piVersion: " 0.84.1", nodeVersion: "24.16.0", pythonMajor: 3 },
        { piVersion: "0.84.1 ", nodeVersion: "24.16.0", pythonMajor: 3 },
        ...
        { piVersion: "0.84.1", nodeVersion: "22.18.0", pythonMajor: 3 },
        { piVersion: "0.84.1", nodeVersion: "24.16.0", pythonMajor: null },
      ]) {
        let apiAccesses = 0;
  ...
  test("exact supported Pi versions and prerequisites return fixed directions", () => {
    ...
    expect(compatibilityDirection({ piVersion: "0.84.1", nodeVersion: "22.18.0", pythonMajor: 3 })).toBe(
      "codeArbiter requires Node >=22.19.0 for Pi; upgrade Node and run /ca-doctor.",
    );
    expect(compatibilityDirection({ piVersion: "0.84.1", nodeVersion: "24.16.0", pythonMajor: null })).toBe(
  ...
  test("a malformed Node version is never treated as satisfying the minimum floor", () => {
    ...
    expect(compatibilityDirection({ piVersion: "0.84.1", nodeVersion: "22.19.0next", pythonMajor: 3 })).toBe(
      "codeArbiter requires Node >=22.19.0 for Pi; upgrade Node and run /ca-doctor.",
    );
  });

  test.each(["0.80.7", "0.84.1-rc.1", "1.0.0"])(
  ...
  test("installed Pi discovery ignores an adjacent package without a Pi executable", async () => {
    ...
    try {
      for (const [packageRoot, version] of [[stalePackage, "0.80.5"], [actualPackage, "0.84.1"]]) {
        await mkdir(resolve(packageRoot, "dist"), { recursive: true });
        await writeFile(
          resolve(packageRoot, "package.json"),
