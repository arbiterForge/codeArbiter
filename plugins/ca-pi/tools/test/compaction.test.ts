...
describe("Task 8 hardened runner seam (RED until Task 7 is stable)", () => {
  ...
  test("canonical validation admits only the installed internal compaction charter", async () => {
    ...
    try {
      ...
      await writeFile(resolve(piRoot, "package.json"), '{"name":"@earendil-works/pi-coding-agent","version":"0.84.1"}\n', "utf8");
      const request = {
        launchKind: "internal-compaction",
      ...
      const dependencies = {
        ...
        packageRoot,
        resolveRuntimeIdentity: async () => ({ cliEntry: piCliPath, packageRoot: piRoot, version: "0.84.1" }),
      };
