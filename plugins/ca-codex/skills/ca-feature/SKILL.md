...
3. **Spec approved, no plan** → `writing-plans` (`${CLAUDE_PLUGIN_ROOT}/routines/writing-plans/SKILL.md`).
4. **Spec exists but never approved** → `brainstorming` (`${CLAUDE_PLUGIN_ROOT}/routines/brainstorming/SKILL.md`), at its approval gate — not from scratch.
...
Then route directly to `tdd` (`${CLAUDE_PLUGIN_ROOT}/routines/tdd/SKILL.md`) — the confirmed criteria are its Phase 1 obligations; Phases 2–6 run
...
1. **`brainstorming`** (`${CLAUDE_PLUGIN_ROOT}/routines/brainstorming/SKILL.md`) — refine `$ARGUMENTS` into a concrete spec by Socratic questioning: challenge
...
2. **`writing-plans`** (`${CLAUDE_PLUGIN_ROOT}/routines/writing-plans/SKILL.md`) — decompose the approved spec into small tasks, each with an exact path and a
...
3. **`executing-plans`** (`${CLAUDE_PLUGIN_ROOT}/routines/executing-plans/SKILL.md`) — coordinates the plan in small batches with human checkpoints. Each batch is
   delegated to `subagent-driven-development` (`${CLAUDE_PLUGIN_ROOT}/routines/subagent-driven-development/SKILL.md` — fresh author agent per task, spec-compliance review,
