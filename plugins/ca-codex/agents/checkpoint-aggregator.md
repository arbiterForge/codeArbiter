...
After writing, list the `### DEFERRABLE` findings as follow-up harvest candidates and signal the orchestrator to run the follow-up harvest ([includes/harvest.md](../includes/harvest.md)) so they reach `open-tasks.md` instead of languishing in the checkpoint doc. This agent does NOT write the board itself — the harvest is confirm-gated and run by the orchestrator (or auto under `/sprint`).
