...
[includes/reviewer-contract.md](../includes/reviewer-contract.md) — the findings format, review output template, gate-status rule, and out-of-scope rule. Read it; do not carry a remembered copy.
...
**Secrets handling:**
- Trace every secret read: does it come from the approved store? Consult the `secret-handling` skill ([routines/secret-handling/SKILL.md](../routines/secret-handling/SKILL.md)) for secret-store policy.
...
Per [includes/reviewer-contract.md](../includes/reviewer-contract.md), plus a `**Control:**` line — the section from `<project-root>/.codearbiter/security-controls.md`. Name the algorithm, the function, the value in the description.
