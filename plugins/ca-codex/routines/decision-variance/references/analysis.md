...
Read [includes/smarts/core.md](../../../includes/smarts/core.md) before Phase 3.

## Pre-flight - scope before prerequisites

Read `<project-root>/.codearbiter/CONTEXT.md` and its `stage:` value. Unreadable or
missing required context is reported as a gap, never initialized by this pass.
Read `<project-root>/.codearbiter/security-controls.md` only when a variance touches
...
Search `<project-root>/.codearbiter/plans/` first, then the project root, then
...
`<project-root>/.codearbiter/decisions/decision-log.md`, and relevant scaffold
...
SHA-256 using [includes/smarts/decision-log-format.md](../../../includes/smarts/decision-log-format.md): heading
