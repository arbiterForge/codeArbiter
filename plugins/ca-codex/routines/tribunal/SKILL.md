...
- Procedure: [routines/tribunal/references/cost-and-models.md](references/cost-and-models.md) — load now.
...
- Produce the inventory (inline, or on a large repo dispatch the optional cheap mappers per [routines/tribunal/references/cost-and-models.md](references/cost-and-models.md)): file tree, language breakdown, entry points/routes, core-logic and shared-utility locations, dependency and integration surface. Write `inventory.md`.
- Apply the judgment overlay in [routines/tribunal/references/ai-markers.md](references/ai-markers.md): risk-rank directories (untrusted input, money, auth, PII, churn = highest), mark trust boundaries, record AI-authorship markers and an iteration-depth estimate. High-marker / high-iteration areas carry a scrutiny boost and a small severity prior.
- Choose the active lenses — the roster IS the set of cards under [routines/tribunal/references/lenses/](references/lenses); the active set is that roster minus any lens whose concern is absent from scope (no migrations → drop the migration lens). Record launched/skipped as `run.jsonl` events.
- Choose the wave partition — the default in [routines/tribunal/references/cost-and-models.md](references/cost-and-models.md), or a repartition for cause — and record it in the `run-started` event ([routines/tribunal/references/schemas.md](references/schemas.md)); resume reads this recorded partition, never re-derives it.
...
Dispatch one `tribunal-lens-reviewer` per active lens, in the wave partition recorded at Phase 1 (default in [routines/tribunal/references/cost-and-models.md](references/cost-and-models.md)) at the concurrency from [routines/tribunal/references/cost-and-models.md](references/cost-and-models.md) (≤5 in flight). Each dispatch carries the assignment block from the agent's Assignment Format — its FIRST line is the title `Tribunal lens: <lens-slug> — <scope summary>` — naming the lens slug, the scope slice, the run dir, and the findings dir, on the model/effort from [routines/tribunal/references/cost-and-models.md](references/cost-and-models.md). The agent itself reads its own mandate from [routines/tribunal/references/lenses/](references/lenses) (the recorded active lens slug plus `.md`, including that card's Required-reading docs) and the finding contract ([routines/tribunal/references/finding-record.md](references/finding-record.md)), and loads neither the other lenses' mandates nor the orchestrator schemas. The orchestrator reads [routines/tribunal/references/finding-record.md](references/finding-record.md) to read findings at triage, and consults a lens mandate only to adjudicate that lens's finding.

- Each dispatched lens reviewer writes each finding to its own file `findings/<lens>/<finding-id>.json` the moment it is found — one file per finding, never a batched write at the end (write contract: [routines/tribunal/references/finding-record.md](references/finding-record.md)).
...
- Procedure: [routines/tribunal/references/triage.md](references/triage.md) — load now.
...
- Procedure: [routines/tribunal/references/issue-filing.md](references/issue-filing.md) — load now.
...
- Procedure: [routines/tribunal/references/telemetry.md](references/telemetry.md) — load now.
