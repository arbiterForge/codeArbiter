...
`<project-root>/.codearbiter/` unless the row gives a `the validated selected-skill root/` path.
...
| Auth, crypto, secrets | `security-controls.md` | `crypto-compliance` / `secret-handling` skill ([routines/<name>/SKILL.md](../routines/<name>/SKILL.md)); `auth-crypto-reviewer` agent |
...
| Following up on workflow residue / a backlog task | `open-tasks.md` (schema), [includes/harvest.md](harvest.md) | `$ca-task` writer; harvest (`extract_*` → `promote`) at a terminal step |
| Generating a user-facing deliverable (UI, report, PR/CHANGELOG prose, slide, chart) | [includes/anti-slop-design/INDEX.md](anti-slop-design/INDEX.md) (then `core` + the medium leaf) | `design-quality-reviewer` agent |
