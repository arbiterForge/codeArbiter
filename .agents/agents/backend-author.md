---
name: backend-author
description: Use when writing or modifying backend/server-side code. Owns the TDD workflow, input validation, framework conventions, ORM usage, and audit event emission. MUST write failing tests before implementation code. Reads tech stack from projectContext/tech-stack.md.
...
1. `projectContext/tech-stack.md` — full read required: language, framework, ORM, test runner command, lint command, test file location convention
2. `projectContext/coding-standards.md` — banned patterns, naming conventions, import style
3. `projectContext/audit-spec.md` — if the feature touches any auditable action (authentication decision, authorization decision, secret read, deployment, role change, schema migration, key rotation, signature verification)
...
2. **Confirm tests fail for the right reason** — run the test command from `tech-stack.md`; the failure message must match the obligation, not an unrelated error
...
- **Audit event emission** — if the action is in `projectContext/audit-spec.md`, a test MUST assert the emit occurs with correct fields
- **Unauthenticated request** — for any API endpoint, a test MUST assert that unauthenticated requests are rejected per the trust zone rules in `projectContext/trust-zones.md`
...
- All input MUST be validated before it touches the database — use the validation library named in `projectContext/tech-stack.md`
...
If the feature performs an auditable action (per `projectContext/audit-spec.md`):

1. Read `audit-spec.md` for the required fields and emit function path
...
- Log errors at the appropriate level per `projectContext/tech-stack.md` logging conventions
