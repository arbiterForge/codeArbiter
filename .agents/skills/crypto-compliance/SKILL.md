...
Read `${PROJECT_ROOT}/.agents/projectContext/security-controls.md` first to determine the banned primitives list for this project. If the file specifies a banned list, use that list. If the file is silent on a primitive, apply the default ban list below.
...
- `curve25519`, `ed25519` (unless `${PROJECT_ROOT}/.agents/projectContext/security-controls.md` explicitly permits them)
...
If `${PROJECT_ROOT}/.agents/projectContext/security-controls.md` explicitly permits a default-banned primitive (other than RSA below 2048), record the explicit permission and proceed. If the file is absent, apply the full default ban list.

Also scan imports for third-party crypto libraries (see Phase 2).

**Gate:** No banned primitives found in changed code (applying project-specific policy read from `${PROJECT_ROOT}/.agents/projectContext/security-controls.md`).

---

### Phase 2 — Allow-List Verification
Read `${PROJECT_ROOT}/.agents/projectContext/security-controls.md` to determine the approved primitives for this project.

If the project specifies FIPS-only mode in `${PROJECT_ROOT}/.agents/projectContext/security-controls.md`, enforce the FIPS-validated allow-list:
...
Read `${PROJECT_ROOT}/.agents/projectContext/security-controls.md` to determine whether a compliance mode check is required.

- If the file specifies **FIPS mode required**: run the FIPS check command specified in `${PROJECT_ROOT}/.agents/projectContext/tech-stack.md`. If no command is specified there, use the project's standard crypto compliance verification. If the check cannot run in the current CI environment, flag the phase as DEFERRED and document in the PR.
...
Read the TLS minimum version from `${PROJECT_ROOT}/.agents/projectContext/security-controls.md`. If the file specifies a minimum TLS version, enforce that. If the file is silent, the default minimum is **TLS 1.3**.
...
- Certificate pinning requirements for sensitive zone connections come from `${PROJECT_ROOT}/.agents/projectContext/security-controls.md` and `${PROJECT_ROOT}/.agents/projectContext/trust-zones.md`.
- Do not bundle or rely on a public CA bundle for internal-network calls — use the project's internal CA bundle as specified in `${PROJECT_ROOT}/.agents/projectContext/security-controls.md`.
...
Read key management requirements from `${PROJECT_ROOT}/.agents/projectContext/security-controls.md`. Enforce the key store the project specifies (e.g., a cloud KMS, an on-premises HSM, an air-gap-suitable key manager).
...
**Gate:** All key operations use the project-approved key store from `${PROJECT_ROOT}/.agents/projectContext/security-controls.md`. No on-disk or unapproved key persistence.
...
Reference the applicable compliance control from `${PROJECT_ROOT}/.agents/projectContext/security-controls.md` in any deviation request.
...
## Hard Rules
- MUST read `${PROJECT_ROOT}/.agents/projectContext/security-controls.md` before Phase 1. BLOCK if the file cannot be read and the project is not a known project with a cached policy.
...
- MUST NOT use third-party crypto libraries unless `${PROJECT_ROOT}/.agents/projectContext/security-controls.md` explicitly permits the named library.
...
- MUST NOT resolve a compliance ambiguity by guessing — read `${PROJECT_ROOT}/.agents/projectContext/security-controls.md` and surface any gap.
