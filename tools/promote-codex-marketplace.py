...
def _catalog_version(raw: bytes) -> str:
    ...
    match = re.fullmatch(r"refs/tags/ca-codex-dist-v(.+)", ref or "")
    version = match.group(1) if match else ""
    if PACKAGER.semver_key(version) is None:
...
def _ledger(previous: bytes | None, *, ref: str, commit: str, source_commit: str,
            cohort_sha256: str, archive_sha256: str) -> bytes:
...
def promote(*, package_root: Path, cohort_sha256: str, source_repo: Path,
            ...
            expected_marketplace_head: str | None=None) -> dict[str, object]:
    ...
    catalog = PACKAGER.promoted_codex_catalog(catalog_source,distribution_url=catalog_url,
        distribution_ref=distribution_ref,distribution_commit=distribution_commit)
    catalog_commit = None
    if advance_channel:
        ...
        ledger = _ledger(previous_ledger,ref=distribution_ref,commit=distribution_commit,
            source_commit=source_commit,cohort_sha256=cohort_sha256,archive_sha256=str(staged["archive_sha256"]))
...
def main(argv=None):
    ...
    print(json.dumps(promote(package_root=a.package_root,cohort_sha256=a.cohort_sha256,source_repo=a.source_repo,
        ...
        push=a.push,advance_channel=a.advance_channel,expected_marketplace_head=a.expected_marketplace_head),indent=2)); return 0
