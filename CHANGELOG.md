...
- **A protected-state registry, enforced as hook class `H-22`.** Project-state
  ...
  The registry ships empty. Consumers enrol in their own later tasks, and it
  stays code constants rather than disk-loaded state, which would otherwise let
  a repo un-protect its own board by editing a file. Recorded as ADR-0024.
