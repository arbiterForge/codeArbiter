...
The arbiter spawns scouts when:
...
```
You are a Scout subagent for the FUSION arbiter skill.

Your scope: <specific paths or areas, e.g., "backend/src/auth/", "helm/fusion/", "docs/adr/0001-0010">

Your task: Scan the assigned scope and report evidence of architectural decisions in these categories from the canonical list in references/decision-categories.md:
...
- **Strong** — explicit evidence (e.g., `import { drizzle } from 'drizzle-orm'` is strong evidence for `STACK.BACKEND.ORM = Drizzle`)
- **Moderate** — implicit evidence (e.g., a Postgres connection string in config is moderate evidence for `STACK.DB.ENGINE = PostgreSQL`)
...
7. **Invent decision categories.** Use only those from `references/decision-categories.md`. Anomalies go in the Anomalies section without category labels.
...
After scouts return, the arbiter composes findings into the unified evidence index (`docs/fusion-arbiter-evidence.md`). Composition is the arbiter's job, not the scouts'. Each scout's report is preserved at the bottom of the evidence index file as an appendix for traceability.
