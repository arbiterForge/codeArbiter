...
After green suite, MUST invoke `security-reviewer` if the change touches any path in security-reviewer's classification matrix (`backend/src/middleware/`, `backend/src/lib/audit/`, any crypto or key file, `deploy/`, `.gitea/workflows/`). Feature is not complete until that review returns.
