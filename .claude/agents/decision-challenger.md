...
After producing the challenge report:

1. For every ADR with confidence ≤ 2: surface the finding immediately to the invoking session. Do not wait for the user to ask. State: "REQUIRES-IMMEDIATE-REVIEW: [ADR title] — confidence [N/5]. Reason: [one sentence]."
2. For every ADR with confidence = 3: note in the summary table that it must be re-challenged before Stage 3 promotion.
3. If any challenge was run because the ADR is AGED (not challenged in 12 weeks): note the date of last challenge and the current gap in weeks.
4. Report to the invoking session: total ADRs challenged, count with confidence ≤ 2, count with confidence = 3.
