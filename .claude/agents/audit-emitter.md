...
For every auditable action with a correct `emit()` call, ALSO verify a test exists that:

1. Mocks emit via `vi.mock('../lib/audit/index.js')`
2. Asserts `mockEmit` was called with correct `action` and `outcome` values
3. Flushes fire-and-forget (`.catch(() => undefined)` pattern covered)
...
<file>:<line> action=<verb.noun>
  ...
  test present:          <yes/no — mock + assert + flush>
