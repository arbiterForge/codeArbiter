...
## Trigger
- Any code change that has different rules at different FUSION stages.
- When the agent needs to know what the current stage requires.
- When stage-tagged rules in CLAUDE.md and docs/ apply differently to the change.
- When code contains stage-conditional logic.

## Phases

### Phase 1 — Stage Read
Run `cat .fusion/stage` and record the integer value. This is the authoritative current stage — do not infer it from code, comments, or config files. If `.fusion/stage` is absent or non-numeric, STOP and surface the error.

**Gate:** `.fusion/stage` read successfully. Integer value recorded.

### Phase 2 — Rule Inventory
List all stage-tagged rules from CLAUDE.md and all `docs/` files that apply at or below the current stage. Rules tagged `[Sn]` apply when current stage ≥ n. Rules tagged `[Sn+]` apply starting at stage n. Untagged rules always apply. Output: table of active rules with their source file.
...
|---|---|---|
| Stage file missing | `.fusion/stage` absent or non-numeric | BLOCK |
...
## Hard Rules
- MUST NOT infer stage from anything other than `cat .fusion/stage`.
...
- MUST NOT reference Python code or Python examples (Python stack retired per ADR-0004).
...
```typescript
const stage = parseInt(await fs.readFile('.fusion/stage', 'utf8').then(s => s.trim()));
```
...
ONLY the `/promote-stage` command (in `.claude/commands/promote-stage.md`) modifies `.fusion/stage`. The agent MUST NOT edit this file directly. Promotion requires:
