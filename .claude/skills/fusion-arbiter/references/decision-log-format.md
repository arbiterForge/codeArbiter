...
This file defines the exact format used in `docs/fusion-arbiter-decisions.md`. The format is non-negotiable so the log remains machine-parseable on future arbiter invocations.
...
If two arbitration sessions race to write decisions, the later session uses the next available number — there is no merge conflict semantics defined for this case (single-user assumption holds for the IR&D phase).
...
A new entry is written with `Status: superseded` only when it is itself the superseding entry — meaning, the new entry has both `Status: superseded` and a `Supersedes: DECISION-NNNN` reference. This is rare; usually a superseding entry is `accepted` and references the prior entry it replaces.

Wait — this is slightly counterintuitive. Let me clarify by example.

## Worked Example: Initial Decision

```markdown
## DECISION-0001 — STACK.AUTH.AUTHORIZATION — OPA bundled, AuthZ-only role
...
- **Artifact position:** OPA bundled, owns all AuthZ; Keycloak owns only AuthN per Architecture Breakdown §3.5–3.6
- **Scaffold position:** silent — no OPA integration in scaffold yet
- **Status type:** scaffold-silent

### Decision

Adopt the artifact position. OPA will be bundled in the FUSION Helm chart and own all authorization decisions. Keycloak handles only identity. The OPA policy bundle will be sourced from the `fusion-policies` Gitea repo.

### SMARTS rationale

Maintainability and Securability dominate. Separating AuthN from AuthZ keeps authorization logic portable across identity providers (lab AD → production OKTA → customer IdP) and centralizes authorization decisions for audit and policy review. Self-contained packaging is preserved.

### Implementation implication

Create scaffold task to bundle OPA in Helm chart. Create `fusion-policies` Gitea repo. Define initial Rego policies for MVP1 role-based access. Spike 4 (OPA Policy Bundle Structure) is required before V1 policy expansion.
...
Months later, the team decides OPA isn't suitable and moves to Cedar:
...
- **Artifact position:** OPA bundled, owns all AuthZ (Architecture Breakdown §3.6, current)
- **Scaffold position:** Cedar policies in `policies/cedar/` directory; OPA removed from Helm chart
- **Status type:** divergent

### Decision

Adopt the scaffold position. Replace OPA with Cedar based on operational experience during MVP1: Cedar's strict typing produced fewer runtime policy errors than expected from OPA's permissive Rego.

### SMARTS rationale

Reliability dominated this revision. OPA Rego policies failed open in two MVP1 incidents due to typos in policy attribute names; Cedar's compile-time validation prevents this class of error. Maintainability remains roughly equivalent. Securable improves due to the typing.

### Implementation implication

Architecture Breakdown §3.6 requires update to reflect Cedar. ADR-0001 (if it captured OPA choice) requires a superseding ADR. `fusion-policies` repo retains its name but now stores Cedar policies. Update enforcement scaffold to validate Cedar instead of OPA bundles.
...
When the arbiter starts a new session, it reads `docs/fusion-arbiter-decisions.md` from top to bottom and indexes:
