---
name: fusion-arbiter
description: Reconcile FUSION (the GDIT solution platform) architectural artifacts (Architecture Breakdown, Phased Build Plan, Task Backlog) against the existing FUSION project scaffold and codebase. Use this skill whenever the user mentions FUSION architectural reconciliation, FUSION variance review, ingesting FUSION decomposition documents, comparing FUSION architectural decisions to existing FUSION code, syncing FUSION artifacts with FUSION scaffold state, or asks to "arbitrate," "reconcile," "merge," or "consolidate" FUSION architectural context with the project. Also use when the user asks what architectural artifacts can be produced from current FUSION project state, requests a FUSION variance report, mentions ADR conflicts in FUSION, or wants to track outstanding FUSION architectural decisions. Do NOT use for routine git merges, code review consolidation, or general architectural discussions outside FUSION. This skill never makes arbitration decisions alone — it presents SMARTS analyses for the user to decide. It treats the artifacts as authoritative-by-default but does not blindly adopt them.
---

# FUSION Arbiter

A skill for systematically reconciling FUSION's three architectural artifacts (Architecture Breakdown, Phased Build Plan, Task Backlog) against the existing project scaffold, codebase, and prior decisions — without making arbitration decisions unilaterally.
...
1. Reads `docs/fusion-arbiter-decisions.md` and extracts the recorded artifact-section hashes for each prior decision
...
- Operational decisions like spike ownership assignments or cross-DA engagement timing (see `references/known-open-decisions.md`)

---

## Pre-Flight Check

Before running the workflow, verify the project is FUSION:

1. Look for any of: `package.json` containing "fusion" in the name field, a `README.md` referencing FUSION as a GDIT project, the three canonical artifact filenames present anywhere in the project, or a `helm/fusion/` directory.
2. If none of these markers can be confirmed, exit with: "This skill is FUSION-specific. The current project does not appear to be FUSION based on the markers I checked. If this is FUSION and the markers are absent, please confirm before I proceed."

This is a runtime safety net that prevents mistaken triggers on unrelated projects.
...
1. **The three architectural artifacts** — by exact filename per Rule 2. Common locations include the project root, `docs/`, `docs/architecture/`, `docs/decomposition/`. Use `find` or equivalent to locate by exact filename. If any are missing, ask the user.

2. **Existing decision records** — look for `docs/adr/`, `docs/decisions/`, `ADRs/`, `architecture/decisions/`, or similar paths. Index every ADR found with its number, title, status, and decision summary.

3. **Existing arbiter decision log** — `docs/fusion-arbiter-decisions.md`. If it exists, this is the persistent record. Read it before generating new variance reports.

4. **The scaffold/codebase itself** — `package.json`, dependency manifests, `backend/`, `frontend/`, `helm/`, `infra/` directories, configuration files, CI configuration.

5. **The arbiter's working files (created if missing):**
   - `docs/fusion-arbiter-evidence.md` — working evidence index, rebuilt every full scan
   - `docs/fusion-arbiter-variance-report.md` — current variance report, overwritten every scan
   - `docs/fusion-arbiter-decisions.md` — persistent decision log, append-only (per Rule 3 hashing)
   - `docs/fusion-arbiter-readiness.md` — downstream artifact readiness, overwritten

If the user's request mentions specific files or sections, prioritize those.

### Stage 1.5 — Stale-Artifact Check (Per Rule 3)

If `docs/fusion-arbiter-decisions.md` exists:
...
Save the evidence index to `docs/fusion-arbiter-evidence.md`. This is a working file, rebuilt on every invocation.
...
For high-variance situations (more than 10 `divergent`/`scaffold-silent`/`artifact-silent` cases), generate the report incrementally — group by area (auth, data, frontend, etc.) and present area-by-area to avoid overwhelming the user.

Save the variance report to `docs/fusion-arbiter-variance-report.md`. This file is overwritten on every full scan.
...
4. After each user decision, immediately append the decision to `docs/fusion-arbiter-decisions.md` — do NOT batch decisions in memory
...
5. Record the decision in `docs/fusion-arbiter-decisions.md` per `references/decision-log-format.md`
...
For any ADR referenced in this variance session that has not been challenged since the last checkpoint run, invoke decision-challenger agent.
...
Save to `docs/fusion-arbiter-readiness.md`. Present the recommendations as a menu. The user picks which to produce.
...
2. **Decision recorded in `docs/fusion-arbiter-decisions.md`** — prior arbitration that has not been superseded
...
Examples of valid scaffold-side pushback:
- "The artifact suggests X, but ADR-0003 already locked Y for documented reasons specific to defense compliance."
- "The artifact assumes a generic OIDC provider, but the scaffold integrates with a specific pattern that has implicit assumptions worth preserving."
...
- `docs/fusion-arbiter-evidence.md` — working evidence index, rebuilt every full scan
- `docs/fusion-arbiter-variance-report.md` — current variance report, overwritten every scan
- `docs/fusion-arbiter-decisions.md` — persistent decision log, append-only with hash tracking per Rule 3
- `docs/fusion-arbiter-readiness.md` — downstream artifact readiness, overwritten
