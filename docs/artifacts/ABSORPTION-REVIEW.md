...
| AC-011 | Partial | Receipt and transition checks plus host-owned exact-prompt user approval pass through installed Claude Code and Codex packages. Verification and independent-review authority adapters exist for Codex and Claude Code (`claude-review/0.1.0`, proven end to end against an engine built from source). Pi prompt, verification and review authority plus SMARTS remain. |
...
| T-013 | Partial | Approval/rebind checks, interactive-user host capture, and verification/review capture mappings exist for Claude Code and Codex; Pi prompt authority, Pi reviewer and verification mappings, and SMARTS remain incomplete. |
