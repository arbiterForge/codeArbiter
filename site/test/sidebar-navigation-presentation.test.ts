    ...
    vi.doMock("node:fs", async (importOriginal) => {
      ...
      return {
        ...
        readFileSync(path: Parameters<typeof readFileSync>[0], options?: Parameters<typeof readFileSync>[1]) {
          if (String(path).includes("/src/generated/sidebar.json")) return "{ malformed";
          return actual.readFileSync(path, options as never);
        },
      };
    });
    try {
      await expect(import(`${pathToFileURL(join(siteRoot, "astro.config.mjs")).href}?malformed=${Date.now()}`))
