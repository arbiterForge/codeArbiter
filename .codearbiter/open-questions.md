...
- **D-7 — Authority-record threat model.** The security review of the Claude authority adapter (H-1) found that request state, observations and authority sources are unkeyed, integrity-hashed files, so a session with filesystem write can forge a passing verification or review, on Codex as well as Claude. That is the ADR-0010 cooperative-agent residual, now recorded in `security-controls.md` §Boundary crossings. Decide whether it stays the accepted model or whether authority receipts move to signed attestation (a key the parent session cannot read) under a new ADR. Non-blocking.
