...
| Host hook evidence to engine receipt | Codex and Claude Code hook events (verifier wrapper Pre/PostToolUse, reviewer launch, child start and first stop) become verification and review observations that the artifact engine captures as `task-review` / `accept-scope` receipts | Each host path binds only closed host fields (Codex `turn_id`/`task_name`; Claude `prompt_id`, `tool_use_id`, `agentId`, first `SubagentStop`), pins its launch envelope and reviewer (`ca:authority-reviewer`, read-only, shadow-checked on Claude), and carries its own engine producer profile so evidence cannot be relabelled across hosts. Pi has no such path. The residual is the cooperative-agent one ADR-0010 already accepts: request state, observations and authority sources live in unkeyed, integrity-hashed files a session with filesystem write can forge, and a parent can plant text a running reviewer reads; this is a governance boundary, not a signed attestation |
